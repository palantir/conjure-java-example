...
/!\/!\/!\/!\/!\/!\/!\/!\/!\/!\/!\/!\/!\/!\/!\/!\/!\/!\
```
The certificates presented with this server are an example so that the tests run in HTTPs. They were generated by following these steps: https://stackoverflow.com/questions/47434877/how-to-generate-keystore-and-truststore
Notes:
...
```
keytool -genkey -alias exampleServerCert -keyalg RSA -keystore keystore.jks -keysize 2048

openssl genrsa -out ca.key 2048
openssl req -new -x509 -days 365 -key ca.key -subj "/C=AQ/ST=AQ/L=AQ/O=Antartica/CN=exampleServerCert" -out ca.crt

openssl req -newkey rsa:2048 -nodes -keyout server.key -subj "/C=AQ/ST=AQ/L=AQ/O=Antartica/CN=exampleServerCert" -out server.csr
openssl x509 -req -extfile <(printf "subjectAltName=DNS:example.com,DNS:www.example.com,DNS:localhost,IP:127.0.0.1") -days 3650 -in server.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out server.crt
```
